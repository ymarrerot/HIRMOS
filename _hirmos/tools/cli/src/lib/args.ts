...
export type ParsedArgs = InitArgs | HelpArgs;
...
export function parseArgs(argv: string[]): ParsedArgs {
  ...
  if (command !== "init") {
    fail(`Unsupported command: ${command}. This CLI currently supports: hirmos init`);
  }

  let projectPath = ".";
  let projectPathSet = false;
  let integrationValue: string | undefined;
  ...
  for (let i = 1; i < argv.length; i += 1) {
    ...
    if (arg === "--integration") {
      const value = argv[++i];
      if (!value) fail("Missing value for --integration");
    ...
    if (arg === "--version") {
      const value = argv[++i];
      if (!value) fail("Missing value for --version");
      version = value;
      continue;
    }
...
export function normalizeVersionOption(value: string): string {
  const normalized = value.trim();
  if (!normalized) fail("--version must not be empty");
  return normalized;
}

export function parseIntegrationList(value: string): string[] {
  const ids = value
    .split(",")
    .map((item) => item.trim())
    .filter(Boolean);
  ...
  if (ids.length === 0) {
    fail("--integration must include at least one integration id");
  }
