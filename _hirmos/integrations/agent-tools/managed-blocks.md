...
The generated HIRMOS block remains the same. `_hirmos/hirmos.config.json` still records each selected integration id.
...
- do not reset unrelated `_hirmos/hirmos.config.json` fields;
- fail clearly if `_hirmos/hirmos.config.json` is invalid JSON.
