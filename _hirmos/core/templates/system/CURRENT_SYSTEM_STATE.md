...
|---|---|---:|---|
| `_hirmos/system/accepted-state/CURRENT_SYSTEM_STATE.md` | Accepted-state navigation authority, current governance pointers, pointer-only current-state navigation, latest accepted close pointer, Work History Ledger outcome index, Source Artifact Index, and compact posture labels | yes | |
...
Canonical pointer table only. This is the navigation spine for future sessions; it must not copy detailed content from source artifacts.

| Source artifact | Owns | Current pointer status | Notes |
|---|---|---|---|
| `_hirmos/system/delivery/DELIVERY_PLAN.md` | roadmap/register | ACTIVE / ACCEPTED / NONE | |
| `_hirmos/system/delivery/<delivery-id>/DELIVERY_SCOPE.md` | delivery authority / close posture | ACTIVE / ACCEPTED / NONE | |
| `_hirmos/system/delivery/<delivery-id>/phases/PHASE-xx.md` | phase authority | ACTIVE / ACCEPTED / NONE | |
| `_hirmos/system/history/sessions/<session-id>/ARCHIVE_MANIFEST.md` | archived close evidence pointer | LATEST / HISTORICAL / NONE | |
| `_hirmos/system/accepted-state/CARRY_FORWARD.md` | approved carry-forward items | ACTIVE / NONE / BLOCKED | |
...
Current System State must remain pointer-only: it may point to Delivery Plan, Delivery Scope, Phase, archive, carry-forward, and evidence locations, but must not duplicate phase progress, delivery status logs, evidence detail, session logs, IU records, carry-forward detail, or accepted-state summaries owned elsewhere.
...
Accepted-state pointer rows, latest accepted close metadata, Work History Ledger outcome index rows, and archive concordance are updated during `hirmos close`. Detailed summaries remain in source artifacts.
...
Pointer tables in this artifact are navigation caches over canonical source artifacts. They may be generated or refreshed from:

- `_hirmos/session/SESSION_STATE.json` for active session/focus pointers;
- `_hirmos/session/SESSION_LEDGER.md` for active boundary and latest gate pointers;
- `_hirmos/system/delivery/DELIVERY_PLAN.md` and delivery directories for delivery/phase authority pointers;
- `_hirmos/system/history/sessions/*/ARCHIVE_MANIFEST.md` for accepted close/history pointers;
- `_hirmos/system/accepted-state/CARRY_FORWARD.md` for active carry-forward pointers.

Do not hand-maintain pointer rows as independent truth. If a pointer row conflicts with an existing source artifact, mark pointer concordance `BLOCKED` and route to reconciliation. If a source artifact does not exist because its concern is not applicable, record `none` / `not created because not applicable`, not a placeholder path.
