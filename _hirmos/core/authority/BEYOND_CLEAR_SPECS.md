# Beyond Clear Specs Authority

*Canonical extension doctrine for when clear specs alone are not sufficient.*

## What this is

This is the authoritative framework pattern for situations where clear specs alone are not reliable enough.

Use this authority file when deciding whether an extension should rely on a clear runnable surface alone or should instead add stronger supporting controls around that surface.

This doctrine has a baseline and an escalation path:
- baseline: clear specs plus local self-validation whenever the run can honestly validate whether the specified behavior was followed;
...
Clear specs alone often fail once an entrypoint must:
...
Spec clarity remains mandatory, but for these cases it is not sufficient.

## When this applies

Apply the baseline form whenever a runnable entrypoint defines concrete executable expectations that can be checked locally before the result is surfaced.
...
The runnable surface and, when needed, the deeper governing local spec or authority must make the target, scope, constraints, acceptance expectations, and known open questions legible enough for a reviewer, operator, or downstream agent to follow without silently guessing. A Clear Spec may be structured, testable, executable, formal, or robust when the situation requires it, but clarity is the foundation.
...
This is mandatory whenever a runnable entrypoint defines concrete executable expectations and can locally validate whether they were actually followed.
...
Even in those lighter cases, keep the baseline when the run can still self-check whether its clear specs were followed.
...
- Does the runnable entrypoint define concrete executable expectations that it can locally self-check?
...
This authority file preserves the HIRMOS doctrine for extending clear specifications with self-validation, governed intermediate structures, governed surfaced-output structures, and fail-closed behavior when the work is synthesis-heavy or trust-sensitive. It is available for future framework hardening and should be applied deliberately when a protocol, command, capability, or artifact boundary needs these controls.
